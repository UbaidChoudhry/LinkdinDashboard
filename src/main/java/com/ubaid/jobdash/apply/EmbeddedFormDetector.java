...
public class EmbeddedFormDetector {
    ...
    /** The job id an embedded Greenhouse board is asked to open: a {@code gh_jid} parameter or {@code Grnhse.Iframe.load(id)}. */
    private static final Pattern GREENHOUSE_JOB_ID = Pattern.compile("gh_jid=(\\d+)|Iframe\\.load\\(\\s*(\\d+)");
    ...
    /**
     * The standalone form URL embedded in the page at {@code pageUrl}, if the page names one.
     * Never throws.
     */
    public Optional<String> detect(String pageUrl) {
        ...
        try {
            return pageFetcher.apply(pageUrl).flatMap(html -> detectInHtml(html, pageUrl));
    ...
    private static Optional<String> greenhouseJobId(String html, String pageUrl) {
        ...
        if (inUrl.find()) {
            return Optional.of(inUrl.group(1) != null ? inUrl.group(1) : inUrl.group(2));
        }
        Matcher inPage = GREENHOUSE_JOB_ID.matcher(html);
        if (inPage.find()) {
            return Optional.of(inPage.group(1) != null ? inPage.group(1) : inPage.group(2));
        }
